...
const updateImageField = (jsonFilePath: string, mediaUrl: string): void => {
  try {
    ...
    throw new Error(
      `Failed to update image field in ${jsonFilePath}: ${error instanceof Error ? error.message : error}`,
    );
  }
};
  ...
  files.forEach((file) => {
    ...
    if (filePath.endsWith(".json")) {
      ...
      try {
        ...
        throw new Error(
          `Failed to update image URL in ${filePath}: ${error instanceof Error ? error.message : error}`,
        );
      }
    } else {
